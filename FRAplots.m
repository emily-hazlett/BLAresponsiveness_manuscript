...
for i = 1:length(neuron)
    ...
    if isfield(neuron(i).PSTH_1msbins, 'FRA_free') % is there a free_FRA
        ...
        fraFreeResponse = zeros(length(fraFree_atten), length(fraFree_freq));
        fraFreeBG = zeros(length(fraFree_atten), length(fraFree_freq));
        for f = 1:length(fraFree_freq)
            for a = 1:length(fraFree_atten)
                ...
                fraFreeResponse (a,f) = mean(sum(psth(window1(1):window1(2), :))); % was 1:end-1. why? changed to :
                fraFreeBG (a,f) = mean(sum(psth(1:window1(1)-1, :)));
                clear psth
            end
        end
        
        fraFreeResponseHz = fraFreeResponse *(1000/ (window1(2)-window1(1)+1));
        fraFreeBGHz = fraFreeBG *(1000/ (window1(1)-1));
        
        fraFreeSMI = zeros(length(fraFree_atten), length(fraFree_freq));
        for f = 1:length(fraFree_freq)
            for a = 1:length(fraFree_atten)
                fraFreeSMI (a,f) = log10((fraFreeResponseHz(a,f)+20)/(fraFreeBGHz(a,f)+20));
            end
        end
        
        output{i, 1} = neuron(i).name;
        output{i, 2} = neuron(i).PSTH.tuning;
        output{i, 3} = log10((mean(fraFreeResponseHz(1,:))+20)/(mean(fraFreeBGHz(1,:))+20));
        %         output{i, 3} = max(max(fraFreeSMI));
        
        neuron(i).PSTH.free = fraFreeResponse;
        neuron(i).PSTH.freeSMI =fraFreeSMI;
    else
    end
    ...
    if isfield(neuron(i).PSTH_1msbins, 'FRA_held') % is there a free_FRA
        ...
        % Calculate mean response for each freq/ atten pair
        fraHeldResponse = zeros(length(fraHeld_atten), length(fraHeld_freq));
        fraHeldBG = zeros(length(fraHeld_atten), length(fraHeld_freq));
        for f = 1:length(fraHeld_freq)
            for a = 1:length(fraHeld_atten)
                ...
                fraHeldResponse (a,f) = mean(sum(psth(window1(1):window1(2), :)));
                fraHeldBG (a,f) = mean(sum(psth(1:window1(1)-1, :)));
                clear psth
            end
        end
        
        fraHeldResponseHz = fraHeldResponse *(1000/ (window1(2)-window1(1)+1));
        fraHeldBGHz = fraHeldBG *(1000/ (window1(1)-1));
        
        fraHeldSMI = zeros(length(fraHeld_atten), length(fraHeld_freq));
        for f = 1:length(fraHeld_freq)
            for a = 1:length(fraHeld_atten)
                fraHeldSMI (a,f) = log10((fraHeldResponseHz(a,f)+20)/(fraHeldBGHz(a,f)+20));
            end
        end
        
        output{i, 4} = max(max(fraHeldSMI));
        
        neuron(i).PSTH.held = fraHeldResponse;
        neuron(i).PSTH.heldSMI = fraHeldSMI;
    else
    end
